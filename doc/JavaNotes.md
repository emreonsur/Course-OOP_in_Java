...
***break deyimi:*** `break` deyimi döngü deyimlerinde ve `switch` deyiminde kullanılabilen bir kontrol deyimidir. `break` deyimi döngüde kullanıldığında döngüyü sonlandırır.
...
`final` veri elemanları ileride ele alınacaktır.
...
### ınterview Question - Integer Decimal Occurence
...
### Değişken Atom İsimlendirme Teknikleri (Variable/Identifier Naming Conventions)

Değişken atom isimlendirmede bazı teknikler kullanılmaktadır. Bu teknikler dışında başka biçimlerde de isimlendirme yapılabilir. Burada anlatılanlar sık kullanılan ve bilinen tekniklerdir:
1. **Unix Style (Snake Case):** Bu isimlendirmede karakterlerin tamamı küçük harfle yazılır, kelimeler alt tire karakteri ile ayrılır. Örneğin: `number_of_devices` `find_file`. Bu isimlendirme Java'da doğrudan tercih edilmez.
2. **Camel Case/Lower Camel Case:** Bu isimlendirmede kelimeler bitişik yazılır. İlk kelimenin baş harfi küçük, geri kalan kelimelerin baş harfi büyük yazılır. Her bir kelimenin ilk harfinden sonra geri kalan tüm karakterleri ise küçük harfle yazılır. Örneğin: `numberOfDevices`, `nextInt`, `findFile`. Bu isimlendirme genel olarak Java'da metot isimlerinde, yerel değişken isimlerinde, parametre değişkeni, sınıf elemanı isimlerinde tercih edilir.
3. **Pascal Case/Upper Camel Case:** Bu isimlendirmede de kelimeler bitişik yazılır. Tüm kelimelerin ilk harfleri büyük, geri kalan tüm karakterleri ise küçük yazılır. Örneğin: `ArrayUtil`, `ReflectionUtils`. Bu isimlendirme Java'da genel olarak UDT isimlerinde tercih edilir.

İsimlendirmede bunlardan biri ya da birden fazlası kullanılabilir. Hatta bunlardan biri kullanılmak zorunda bile değildir. Örneğin Java'da paket isimlerinin tamamı küçük harf ve kelimeler bitişik olacak şekilde yazılır. Örneğin: `java.util`, `org.csystem.util.array`, `org.sprinframework.boot`.
Burada anlatılan teknikler tamamen geneldir ve bunlara isimler verilmiştir. Hatta bazen isimlendirmede bir ya da birden fazla teknik biraz değiştirilerek bile kullanılabilmektedir.

Programcılar genel olarak programlama dilinin doğasına uygun olacak şekilde isimlendirme yaparlar. Örneğin JavaSE'de metot isimleri lower camel case olarak bildirildiğinden Java programcısı da kendi yazdığı metotlar için aynı teknikle isimlendirme yapar.

### Encapsulation, Data/Information Hiding

NYPT'de özellikle veri elemanlarının gizlenmesine **encapsulation** denilmektedir. Bu anlamda bu kavrama **veri/bilgi gizleme (data/information hiding**) de denilmektedir. Bu kavram aslında gerçek hayattan programlamaya aktarılmıştır. Örneğin, televizyon izleyen bir kişi televizyonun, kumandası ile nasıl haberleştiğini bilmek zorunda değildir. Yani bunu bilmesi ya da bilmemesi kişinin televizyon izlemesini etkileyen bir durum değildir. Bu durumda aslında televizyonun kumanda ile haberleşme işlemi televizyonu izleyen/kullanan kişiden gizlenmiştir. Çünkü, başka bir şekilde söyleyecek olursak, televizyonun izlenmesi için bu bilginin bilinmesine gerek yoktur. Ancak televizyonu üreten taraf açısından bunun bilinmesi gerekir.

Bir sınıf için iki bakış açısı söz konusudur:
- **Yazan bakış açısı (server)**
- **Kullanan bakış açısı (client)**

Sınıfı yazan taraf, sınıfa ilişkin tüm detaylarla birlikte sınıfı kullanacak tarafın bakış açısını da bilmelidir. Sınıfı kullanan ise içsel detayları bilmek zorunda değildir. Bu anlamda sınıfın bildirimine hizmet veren kodlar için **server codes**, sınıfı kullanan kodlara ise hizmet alan anlamında **client codes** denilmektedir. Aslında bu kavramlar geneldir. Sınıf elemanlarının gizlenmesi ile NYPT'de kullanılmaktadır.

Bir sınıfın veri elemanı gizlendiğinde, dışarıdan değerinin değiştirilmesi ve/veya değerinin elde edilmesi gerekebilir. Bunun için veri elemanına erişen `public` metotların yazılması gerekir. Gizlenmiş bir veri elemanının değerini değiştirmek için yazılan `public` metoda **set metodu (setter/mutator)** denir ve bir convention olarak (genelde) `set` ön eki ile başlatılır. Gizlenmiş bir veri elemanının değerini elde etmek için yazılan `public` metoda ise **get metodu (getter/accessor)** denir ve bir convention olarak (genelde) `get` ön eki ile başlatılır. Veri elemanı `boolean` türdense, *getter* tipik olarak `is` ile başlatılır. Gizlenen bir eleman için *mutator* ve *accessor* metotların yazılıp yazılmayacağı sınıfa yani domain'e bağlıdır. Bazı veri elemanları için hiçbiri yazılmayabilir iken, bazı veri elemanları için bir tanesi, bazıları için ise ikisi de yazılabilir.

***Anahtar notlar:*** Bir sınıfın dokümante edilen kısımları `public` ve `protected` bölümleridir. `private` ve no-modifier bölümleri dokümante edilmez. Bir sınıfın bir elemanının erişim belirleyicisi (yani aslında sınıfın bölümleri) diğer sınıflar için anlamlıdır. Sınıf içerisinden erişimde elemanın hangi bölümde olduğunun önemi yoktur.

***Anahtar notlar:*** Bazen `private` veri elemanlarının isimlendirilmesinde birtakım ön eklerin (prefix) kullanıldığı görülebilir. Örneğin, `m_`, `d_`, `m` gibi ön ekler tercih edilebilmektedir. Bazı programcılar hiçbir ön ek koymamayı tercih edebilirler. Bu tür yaklaşımların iyisi kötüsü yoktur. Geliştirici ekip tarafından nasıl bir teknik belirlendiyse o kullanılır. Biz convention olarak, sınıfların non-static ve `private` veri elemanları için `m_` ön ekini, `static` ve `private` veri elemanları için ise `ms_` ön ekini kullanacağız.

> Aşağıdaki demo `Time` sınıfı ile birlikte sınıfa ait accessor ve mutator metotları inceleyiniz.
...
Aşağıdaki durumlardan en az bir tanesi söz konusu ise ilgili veri elemanı gizlenir:
- Sınıfların versiyonları ilerledikçe, yani sınıfın kodlarında değişiklikler ya da eklentiler yapıldıkça veri elemanlarının isimlerinin hatta türlerinin değiştirilmesi ile çok fazla karşılaşılır. Bu durumda eski kodların yeni değişikliklerden etkilenmemesi için veri elemanları gizlenir.
...
***Anahtar notlar:*** Bazı durumlarda yukarıdaki durumlardan biri olmasa bile veri elemanı gizlenebilir. Örneğin, sınıfın bir veri elemanı dışında kalan diğer veri elemanlarının hepsinin gizlenmesi gerekebilir. Bütünlüğün bozulmaması için gizlenmesi gerekmeyen veri elemanı da gizlenebilir. Bu durumda yine ilgili accessor ve mutator metotlar yazılır. Bu da aslında sınıfa bağlıdır.

***Anahtar notlar:*** Yukarıdaki durumlardan hiçbirisi ile karşılaşılmaması durumunda (%3 - %5 olasılıkla karşılaşılmaz.) ilgili veri elemanı `public` yapılabilir.

> Aşağıdaki demo `Time` sınıfının kodlarının değişmesine ve `App` sınıfının `main` metodu içerisindeki client code'da bir değişiklik yapılmamış olmasına rağmen kodun etkilenmediğine dikkat ediniz.
...
***Anahtar notlar:*** Bazı durumlarda sınıfın `public` bölümünde değişiklik yapılması gerekebilir. Bu durumda değişikliği doğrudan yapmak yerine, eskisi korunup `deprecated` yapılabilir. Şüphesiz bu durum da domain'e bağlıdır. Ayrıca bu durum sınıfın dokümantasyonunda belirtilir.

## 8 Ekim 2024

Bir metodun sınıf dışından çağrılması istenmiyorsa, yani genel olarak sınıf dışından çağrılması anlamsızsa bu durumda ilgili metot gizlenir.

> Circle sınıfının aşağıdaki implementasyonunda `calculate` metodunun sınıf dışından çağrılmasının bir anlamı olmayacağından `private` yapıldığında dikkat ediniz.
...
NYPT için birtakım ilkeler söz konusudur. Bu ilkelere literatürde **Object Oriented Design Principles** da denilmektedir. Aslında bu ilkeler yalnızca NYPT'ye özgü değildir. NYPT için de uygulanabilirler.

Kısaca **SOLID** olarak bilinen bu ilkeler şunlardır:
...
**Not:** Bu ilkelere ilişkin detaylar bu kurs ve uygulama kursları ile birlikte konular içerisinde anlaşılacaktır.

#### Single Responsibility Principle

Bu ilkeye göre bir sınıfın tek bir sorumluluğu olmalıdır. Bu anlamda bir sınıf birbirinden bağımsız işler yapacak şekilde tasarlanmamalıdır. Bu ilkeye ilişkin iki temel motto şu şekildedir:
...
Özellikle NYPT ilerledikçe bazı programcılar birtakım problemlere genel çözümler üretme çabasına girmişlerdir. Bu anlamda **Gang of Four (GoF)** olarak anılan 4 bilim insanı bir grup genel problemi çözümleriyle birlikte kategorize etmişlerdir. Bu tip genel çözümlere **tasarım kalıpları (design patterns)** denilmektedir. GoF pattern'lerinden sonra da pek çok pattern tasarlanmıştır. Bu kurs ve uygulama kurslarında çeşitli pattern'ler konular içerisinde ele alınacaktır.

## Singleton Tasarım Kalıbı

Singleton GoF'un nesne yaratan (creational) kalıplarından biridir. Bu tasarım kalıbının (design pattern) çözüm olduğu problemin tanımı şu şekilde verilebilir: **Öyle bir sınıf olsun ki o sınıf türünden program boyunca yalnızca bir tane nesne yaratılabilsin ve istendiği zaman o nesne referansı elde edilebilsin.**

Singleton kalıbının bir çok implementasyonu söz konusudur. Zaman içerisinde bunlar ele alınacaktır. Ayrıca burada Singleton kalıbının hangi durumlarda kullanılacağı ele alınmayacaktır. Yalnızca probleme ve çözümüne odaklanınız.
...
> `Point` sınıfını **kutupsal koordinatlar (polar coordinates)** kullanılarak da nesne yaratılabilecek şekilde güncellemek isteyelim. Bu durumda kutupsal koordinata ilişkin yarıçapı ve açı bilgisini alan bir ctor ile kartezyen koordinatları alan iki ctor aynı imzaya sahip olduklarından ikisi birden `Point` sınıfı içerisinde yazılamaz. Bu durumda `Point` sınıfına aşağıdaki şekilde 3 parametreli bir ctor ekleme gibi bir yaklaşıma gidilebilir:
...
Kutupsal koordinatlara ilişkin detaylı bilgiye [buradan](https://en.wikipedia.org/wiki/Polar_coordinate_system) ulaşabilirsiniz.
...
Sınıfın `static` ve `final` olarak bildirilmiş bir veri elemanına aşağıdaki iki yerden yalnızca birinde değer verilebilir:
- Bildirim noktasında (initialization)
